...
import com.example.drawerListeners.OnDrawerCloseListener;
import com.example.drawerListeners.OnDrawerOpenListener;
import com.example.drawerListeners.OnDrawerScrollListener;
import com.example.hw3.R;

/**
 * DrawerViewGroup hides content out of the screen and allows the user to drag a
 * handle to bring the content on screen. DrawerViewGroup can be used vertically
 * or horizontally.
 *
 ...
 *
 * DrawerViewGroup should be used as an overlay inside layouts. This means
 * DrawerViewGroup should only be used inside of a FrameLayout or a RelativeLayout
 * for instance. The size of the DrawerViewGroup defines how much space the
 * content will occupy once slid out so DrawerViewGroup should usually use
 * match_parent for both its dimensions.
 *
 * Inside an XML layout, DrawerViewGroup must define the id of the handle and of
 * the content:
 *
 * <pre class="prettyprint">
 * &lt;com.hollowsoft.library.DrawerViewGroup.DrawerViewGroup
 ...
 * </pre>
 *
 * @attr ref R.styleable#DrawerViewGroup_animateOnClick
 * @attr ref R.styleable#DrawerViewGroup_allowSingleTap
 * @attr ref R.styleable#DrawerViewGroup_topOffset
 * @attr ref R.styleable#DrawerViewGroup_bottomOffset
 * @attr ref R.styleable#DrawerViewGroup_orientation
 * @attr ref R.styleable#DrawerViewGroup_handle
 * @attr ref R.styleable#DrawerViewGroup_content
 *
 * <p> This class has ported and improved from the Android Open Source Project.
 *
 * @see <a href="http://http://developer.android.com/reference/android/widget/DrawerViewGroup.html">
 *      DrawerViewGroup</a>
 *
 * @author Igor Morais
 * @author mor41s.1gor@gmail.com
 */
public class DrawerViewGroup extends ViewGroup {
	...
	private final int zoomInResId;
	private final int zoomOutResId;
	...
	/**
	 * Creates a new DrawerViewGroup from a specified set of attributes defined in
	 * XML.
	 *
	 * @param context
	 ...
	 */
	public DrawerViewGroup(final Context context, final AttributeSet attributeSet) {
	 ...
	 */
	public DrawerViewGroup(final Context context, final AttributeSet attributeSet, final int defStyle) {
		super(context, attributeSet, defStyle);

		final TypedArray typedArray = context.obtainStyledAttributes(attributeSet,
				R.styleable.DrawerViewGroup, defStyle, 0);

		animateOnClick = typedArray.getBoolean(R.styleable.DrawerViewGroup_animateOnClick, true);
		allowSingleTap = typedArray.getBoolean(R.styleable.DrawerViewGroup_allowSingleTap, true);
		topOffset = (int) typedArray.getDimension(R.styleable.DrawerViewGroup_topOffset, 0.0f);
		bottomOffset = (int) typedArray.getDimension(R.styleable.DrawerViewGroup_bottomOffset, 0.0f);

		final int orientation = typedArray.getInt(R.styleable.DrawerViewGroup_orientation,
				ORIENTATION_VERTICAL);

		isVertical = orientation == ORIENTATION_VERTICAL;

		handleResId = typedArray.getResourceId(R.styleable.DrawerViewGroup_handle, Integer.MIN_VALUE);
		if (handleResId == Integer.MIN_VALUE) {
			throw new IllegalArgumentException(
					"The handle attribute is required and must refer to a valid child.");
		}

		contentResId = typedArray.getResourceId(R.styleable.DrawerViewGroup_content, Integer.MIN_VALUE);
		...
		zoomInResId = typedArray.getResourceId(R.styleable.DrawerViewGroup_zoomIn, Integer.MIN_VALUE);
		zoomOutResId = typedArray.getResourceId(R.styleable.DrawerViewGroup_zoomOut, Integer.MIN_VALUE);
	...
	protected void onFinishInflate() {
		...
		zoomInView = findViewById(zoomInResId);
		zoomOutView = findViewById(zoomOutResId);
	...
	protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
		...
		if (widthSpecMode == MeasureSpec.UNSPECIFIED || heightSpecMode == MeasureSpec.UNSPECIFIED) {
			throw new IllegalStateException("The DrawerViewGroup cannot have unspecified dimensions.");
		}
		...
		if (isVertical) {
			...
			zoomInView.measure(MeasureSpec.makeMeasureSpec(widthSpecSize, MeasureSpec.UNSPECIFIED),
					MeasureSpec.makeMeasureSpec(height, MeasureSpec.UNSPECIFIED));
			zoomOutView.measure(MeasureSpec.makeMeasureSpec(widthSpecSize, MeasureSpec.UNSPECIFIED),
					MeasureSpec.makeMeasureSpec(height, MeasureSpec.UNSPECIFIED));
			...
			zoomInView.measure(MeasureSpec.makeMeasureSpec(width, MeasureSpec.UNSPECIFIED),
					MeasureSpec.makeMeasureSpec(heightSpecSize, MeasureSpec.UNSPECIFIED));
			zoomOutView.measure(MeasureSpec.makeMeasureSpec(width, MeasureSpec.UNSPECIFIED),
					MeasureSpec.makeMeasureSpec(heightSpecSize, MeasureSpec.UNSPECIFIED));
		}
	...
	protected void onLayout(final boolean changed, final int left, final int top, final int right,
		...
		if (!isTracking) {
			...
			if (isVertical) {
				...
				zoomInView.layout(0, topOffset + handleMeasuredHeight, zoomInView.getMeasuredWidth(),
						topOffset + handleMeasuredHeight + zoomInView.getMeasuredHeight());
				zoomOutView.layout(zoomInView.getMeasuredWidth(), topOffset + handleMeasuredHeight,
						zoomOutView.getMeasuredWidth() + zoomInView.getMeasuredWidth(),
						topOffset + handleMeasuredHeight + zoomOutView.getMeasuredHeight());
				...
				zoomInView.layout(topOffset + handleMeasuredWidth, 0, topOffset + handleMeasuredWidth
						+ zoomInView.getMeasuredWidth(), zoomInView.getMeasuredHeight());
				zoomOutView.layout(topOffset + handleMeasuredWidth, 0, topOffset + handleMeasuredWidth
						+ zoomOutView.getMeasuredWidth(), zoomOutView.getMeasuredHeight());
			}
	...
	protected void dispatchDraw(final Canvas canvas) {
		...
		if (isTracking || isAnimating) {
			...
			if (bitmap == null) {
				...
				drawChild(canvas, zoomInView, drawingTime);
				drawChild(canvas, zoomOutView, drawingTime);
	...
	public void onInitializeAccessibilityNodeInfo(final AccessibilityNodeInfo nodeInfo) {
		...
		nodeInfo.setClassName(DrawerViewGroup.class.getName());
	}
	...
	public void onInitializeAccessibilityEvent(final AccessibilityEvent event) {
		...
		event.setClassName(DrawerViewGroup.class.getName());
	}
	...
	private void closeDrawer() {
		...
		zoomInView.setVisibility(View.GONE);
		zoomInView.destroyDrawingCache();
		zoomOutView.setVisibility(View.GONE);
		zoomOutView.destroyDrawingCache();
	...
	/**
	 * Locks the DrawerViewGroup so that touch events are ignores.
	 *
	 * @see #unlock()
	 */
	...
	/**
	 * Unlocks the DrawerViewGroup so that touch events are processed.
	 *
	 * @see #lock()
	 */
